...
```{r LoadPack, message = FALSE, echo = FALSE}
library(ggplot2)
library(gplots)
library(dplyr)
library(statsr)
...
```{r pairstat, echo=FALSE, eval=FALSE}
### For some reason these codes do not produce the output of the codes below it

coneduc <- gss %>% filter(year != "NA", coneduc != "NA", nateduc != "NA") %>% filter(year >=2000) %>% group_by(year,coneduc) %>% summarise(n_coneduc = n()) %>% mutate(total = sum(n_coneduc), perc_conf = n_coneduc/total) %>% mutate(perc_conf = round(perc_conf*100,1)) %>% mutate(category = rep("confidence", 3)) %>% rename(response = coneduc, n = n_coneduc)
nateduc <- gss %>% filter(year != "NA", nateduc != "NA", coneduc != "NA") %>% filter(year >=2000) %>% group_by(year,nateduc) %>% summarise(n_nateduc = n()) %>% mutate(total = sum(n_nateduc), perc_conf = n_nateduc/total) %>% mutate(perc_conf = round(perc_conf*100,1)) %>% mutate(category = rep("spending", 3))  %>% rename(response = nateduc, n = n_nateduc)
educ_df <-  rbind(coneduc, nateduc)
...
```{r sumstat, echo=TRUE, fig.width=13}
...
```{r chisq1, echo=TRUE, fig.width=13}
...
```{r pltms, echo=FALSE, fig.align='center'}
plotmeans(coninc~nateduc, connect= 1, col="red", main = "Plot of means with 95% CIs", xlab="levels of confidence", ylab="Total family income in constant dollars", mean.labels = TRUE, ci.label = TRUE, ylim = c(20000, 80000), cex = 0.75, data = inc)
```
...
```{r gghistog, echo=TRUE, fig.width=13}
...
The number of respondents in Too Little group is *`r genprop_tl$total[2]`* and the proportion of females are *`r genprop_tl$perc[2]`*. The number of successes or the number of females are *`r genprop_tl$count[2]`*. The  number of failures or the number of males are *`r genprop_tl$count[1]`*.

The number of respondents in About Right group is *`r genprop_ar$total[2]`* and the proportion of females are *`r genprop_ar$perc[2]`*. The number of successes or the number of females are *`r genprop_ar$count[2]`*. The  number of failures or the number of males are *`r genprop_ar$count[1]`*.

The success failure condition is met.

###Create a 95% confidence interval

```{r confinterv, echo=TRUE}
...
The proportion of women in the group too little is 53% while on the group about right the proportion is 49%,  a difference of 4%. Our 95% confidence interval for this point estimate is **`r confI`**.  The interval contains the null value of 0, which means that we cannot discount the possiility that there is no difference between the two proportions.
...
##Pooled proportion
```{r poolprop, echo=TRUE}
pooled_prop <- (genprop_tl$count[2] + genprop_ar$count[2])/(genprop_tl$total[2] + genprop_ar$total[2])
pooled_prop
```

The number of respondents in the group too little are *`r genprop_tl$total[2]`* and the  pooled proportion *`r pooled_prop`*. The  number of successes are *`r round(genprop_tl$total[2]*pooled_prop,1)`*. The  number of failures are *`r round(genprop_tl$total[2]*(1-pooled_prop),1)`*.

The number of respondents in the group about right are *`r genprop_ar$total[2]`* and the  pooled proportion *`r pooled_prop`*. The  number of successes are *`r round(genprop_ar$total[2]*pooled_prop,1)`*. The  number of failures are *`r round(genprop_ar$total[2]*(1-pooled_prop),1)`*.
...
```{r proptest, echo=TRUE, fig.width=13}
